...
const triggerProfileActions = async () => {
    ...
    const appDetails = document.getElementById('appDetailsSection');

    // Hide application-specific content
    if(personal) personal.style.display = 'none';
    if(academic) academic.style.display = 'none';
    if(appDetails) appDetails.style.display = 'none';
    
    header.innerHTML = ""; // Clear the student name/ID header
    document.getElementById('breadcrumbActive').innerText = "Staff Settings";

    body.innerHTML = `
        <div style="text-align: center; padding: 40px 20px; max-width: 400px; margin: 0 auto;">
            <div style="width: 80px; height: 80px; background: #e3f2fd; color: #4a90e2; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; font-size: 2rem;">
                <i class="fas fa-user-shield"></i>
            </div>
            <h2 style="font-size: 1.4rem; margin-bottom: 5px;">Staff Account</h2>
            <p style="color: #666; margin-bottom: 30px; font-size: 0.9rem;">${user.email}</p>
            
            <div style="display: flex; flex-direction: column; gap: 15px;">
                <button id="btnResetPass" style="padding: 12px; background: white; border: 1px solid #4a90e2; color: #4a90e2; border-radius: 8px; cursor: pointer; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 10px;">
                    <i class="fas fa-key"></i> Reset Password
                </button>
                <button id="btnLogout" style="padding: 12px; background: #fff0f0; border: 1px solid #ff4d4d; color: #ff4d4d; border-radius: 8px; cursor: pointer; font-weight: 600; display: flex; align-items: center; justify-content: center; gap: 10px;">
    ...
    document.getElementById('btnResetPass').onclick = () => {
        sendPasswordResetEmail(auth, user.email)
            .then(() => alert("A password reset link has been sent to: " + user.email))
