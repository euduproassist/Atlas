...
import { doc, setDoc, getDoc, query, where, collection, getDocs } from "https://www.gstatic.com/firebasejs/11.0.0/firebase-firestore.js";
import { getStorage, ref, uploadBytes, getDownloadURL } from "https://www.gstatic.com/firebasejs/11.0.0/firebase-storage.js";
const storage = getStorage();

const POLICY_TEXTS = {
    admission: `1. GENERAL DECLARATION AND ACCURACY OF INFORMATION\n1.1. I, the applicant, hereby certify that all information provided in this application, including all supporting documentation uploaded to the Document Vault, is to the best of my knowledge true, accurate, and complete.\n1.2. I acknowledge that the institution reserves the right to cancel my application, withdraw any offer of admission, or terminate my registration at any stage should it be discovered that any information provided is false, misleading, or fraudulent.\n1.3. I understand that "False Info" is a category for immediate rejection and may result in a permanent ban.\n\n2. DATA PRIVACY AND PROTECTION OF PERSONAL INFORMATION (POPIA)\n2.1. In accordance with the Protection of Personal Information Act (POPIA), I hereby grant consent to the institution to process my personal data.\n2.2. I understand that this data will be used strictly for the purposes of admissions, academic administration, and institutional planning.\n\n3. ADMISSION REQUIREMENTS AND SELECTION CRITERIA\n3.1. I acknowledge that meeting the minimum Admission Point Score (APS) does not guarantee admission.\n3.2. I understand that my "1st Choice" is my primary preference.\n3.3. I accept that "Provisional Acceptance" is conditional.\n\n4. DOCUMENTATION AND THE DOCUMENT VAULT\n4.1. I agree to ensure that all documents uploaded are clear and legible.\n4.2. I understand that if my application status is marked as "MISSING INFO," it is my responsibility to monitor the portal.\n\n5. COMMUNICATION AND MONITORING\n5.1. I acknowledge that the primary mode of communication is via this Student Portal.\n\n6. FINANCIAL LIABILITY AND SPONSORSHIP\n6.1. I acknowledge that submission of this application may require an application fee.\n\n7. DISABILITY AND SPECIAL ACCOMMODATIONS\n7.1. I certify that the details provided in the disability section are intended to facilitate my academic success.\n\n8. RULES AND REGULATIONS\n8.1. By clicking "Submit," I agree to abide by the rules, regulations, and policies of the institution.`,
    
    rules: `1. ADMISSION AND REGISTRATION POLICIES\n1.1. Accuracy of Information: The Institution reserves the right to cancel any application if fraudulent documentation is provided.\n1.2. APS and Selection: Admission is subject to meeting minimum APS and space availability.\n1.3. Document Vault Compliance: Failure to provide "Missing Info" within 14 days results in withdrawal.\n\n2. DISCIPLINARY RULES AND STUDENT CONDUCT\n2.1. General Conduct: Every student shall maintain a standard of conduct that does not bring the Institution into disrepute.\n2.2. Academic Integrity: Plagiarism and cheating constitute gross misconduct.\n\n3. FINANCIAL POLICIES AND FEES\n3.1. Liability for Fees: A student becomes liable for fees upon registration.\n3.2. Application Fees: The application fee is non-refundable.\n\n4. DATA PRIVACY (POPIA)\n4.1. Consent: By utilizing the portal, the student consents to data processing for academic and financial administration.\n\n5. RESIDENCE AND ACCOMMODATION\n5.1. Eligibility: Academic acceptance does not guarantee housing.\n\n6. DECLARATION OF COMMITMENT\nI acknowledge that I have read and understood the Institutional Rules and Regulations.`,
    
    sharing: `1. PREAMBLE AND SCOPE\nIn accordance with POPIA, the Institution requires consent to share data with verified third-party providers for support services.\n\n2. NATURE OF DATA TO BE SHARED\nInformation shared includes identifiers, contact details, academic records, and demographic data.\n\n3. AUTHORIZED THIRD-PARTY RECIPIENTS\nRecipients include Career Portals, Housing Providers, Financial Institutions (NSFAS), and Technical Partners (Paystack/Firebase).\n\n4. PURPOSE OF PROCESSING\nTo verify enrollment, streamline employment transitions, and fulfill statutory reporting (DHET).\n\n5. DATA SECURITY AND RETENTION\nThe Institution employs encryption and Firebase security rules to protect data.\n\n6. STUDENT RIGHTS\nYou have the right to access records of data sharing and object to specific processing.\n\n7. DECLARATION\nI grant the Institution permission to share my information with authorized third-party providers.`
};
...
if (activeCycle) {
    ...
    const coursesSnap = await getDocs(query(collection(db, "course_offerings"), where("cycleId", "==", activeCycle.id)));
    let allCourses = [];
    ...
    campusSelect.addEventListener('change', () => {
        ...
        if (selectedCampus) {
            const validCourses = allCourses.filter(c => c.attendanceMode === selectedMode && c.campus === selectedCampus);
            courseSelect.innerHTML += validCourses.map(c => `<option value="${c.course}">${c.course}</option>`).join('');
...
window.renderReviewSummary = async function() {
    ...
    if (docSnap.exists()) {
        ...
        summaryDiv.innerHTML = `
            ...
            <p><strong>Academic Year:</strong> ${data.academicYear || (activeCycle ? activeCycle.academicYear : 'N/A')}</p>
