...
import { doc, getDoc, onSnapshot, updateDoc, query, collection, getDocs, where } from "https://www.gstatic.com/firebasejs/11.0.0/firebase-firestore.js";
...
document.getElementById('trackStatusBtn').addEventListener('click', async () => {
    ...
    <tr style="border-bottom: 1px solid #eee;">
      ...
      data.documentStatuses && Object.values(data.documentStatuses).includes('rejected') ? '<span style="color:#d32f2f">ACTION REQUIRED</span>' :
...
`;
    ...
    ${f.readOnly ? 
        `<span style="color: #4a90e2; font-weight: 800; font-size: 0.75rem;"><i class="fas fa-lock"></i> READ ONLY</span>` : 
    ...
    <div>
        ...
        ${data.studentNumber ? `<span style="color: #2e7d32; font-size: 0.8rem; font-weight:bold;">Student No: ${data.studentNumber}</span>` : ''}
    </div>
    <div>
        <span style="color: #888; font-size: 0.85rem;">Name:</span>
        <strong style="display: block;">${s1.fullNames || ''} ${s1.surname || ''}</strong>
